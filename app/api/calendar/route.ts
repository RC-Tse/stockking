...
export async function GET(req: NextRequest) {
  ...
  const dailyStats: Record<string, { pnl: number; pnl_pct: number; note: string }> = {}
  ...
  for (let d = new Date(startD); d <= endD; d.setDate(d.getDate() + 1)) {
    ...
    if (hasPrice || todaysTxs.length > 0) {
      ...
      const denominator = prevMarketValue + capitalIn
      ...
      if (hasPrice) {
        prevUnrealizedPnl = currentUnrealizedPnl
        prevMarketValue = currentMarketValue
      }
    }
  }

  return NextResponse.json(Object.entries(dailyStats).map(([date, data]) => ({
    entry_date: date,
    pnl: data.pnl,
