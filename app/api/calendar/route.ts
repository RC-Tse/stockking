    ...
    if (isCurrentMonth) {
      ...
      if (Math.abs(dailyMarketPnl) > 0.01 || Math.abs(dailyRealizedPnl) > 0.01 || todaysTxs.length > 0) {
        dailyStats[date] = {
          ...
          details,
          note: todaysTxs.length > 0 ? `äº¤æ?: ${todaysTxs.map(t => `${t.action} ${t.symbol}`).join(', ')}` : ''
        }
      }
    }
  }
