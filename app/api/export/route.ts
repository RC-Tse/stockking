...
export async function GET(req: NextRequest) {
  ...
  // Sheet 3: 損益摘要 (Based on total history)
  ...
  const summary = Object.keys(stats).map(sym => {
    ...
    return {
      ...
      '目前持股數': heldShares,
      '目前市值(參考)': mv,
      '未實現損益(參考)': Math.round(mv - heldCost)
    }
  })
  ...
  XLSX.utils.book_append_sheet(wb, ws1, '自行交易')
  ...
  XLSX.utils.book_append_sheet(wb, ws3, '損益摘要')
