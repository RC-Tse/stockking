      ...
      <div className="flex items-end justify-between px-4">
        <div className="space-y-1">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-[var(--t1)] font-mono">
        ...
        <div className="text-right">
          ...
          <div className={`text-3xl font-black font-mono ${currentValue >= 0 ? 'text-red-400' : 'text-green-400'}`}>
             {Math.abs(currentValue) >= 1000 ? `${currentValue >= 0 ? '+' : '-'}${(Math.abs(currentValue) / 1000).toLocaleString()}K` : fmtMoney(currentValue)}
          </div>
        </div>
      </div>

      <div className="bg-[var(--bg-card)] border border-[var(--border-bright)] rounded-3xl p-0 shadow-2xl relative overflow-hidden group">
        ...
        >
          ...
          >
            <svg viewBox={`0 0 ${viewBoxWidth} ${chartHeight}`} className="w-full h-full overflow-visible">
              <g>
                ...
                {data.map((d, i) => {
                  ...
                  const isJan = month === 1
                  const label = (isCrossYear && isJan) ? `${year}/${month}` : `${month}/${parseInt(day)}`
            ...
            {isScrubbingMode && activeIdx !== null && data[activeIdx] && !data[activeIdx].isFuture && (
              ...
              >
                <div className="glass p-5 border-white/10 shadow-2xl backdrop-blur-3xl rounded-3xl min-w-[200px]">
                  <div className="text-[12px] text-white/60 font-black mb-4 uppercase tracking-widest">{data[activeIdx].date}</div>
