...
function IntegratedCalendar({ entries, transactions, onRefresh, holdings, quotes, settings }: any) {
  ...
  const toggleDate = async (dateStr: string) => {
    ...
    if (isWeekend) { setIsHoliday(true); setDayDetails([]); setLoading(false); return }

    try {
      const held = holdings.map((h: any) => h.symbol)
      if (held.length === 0) { setDayDetails([]); setLoading(false); return }
    ...
    >
      ...
      {entry && (
        <div className="flex-1 flex flex-col justify-center items-center mt-1">
          {/* Daily PnL Container using flex-1 to distribute space */}
