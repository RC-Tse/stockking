...
function YearlyPnLChartContent({ transactions, settings, year }: Props) {
  ...
  const handleTouchStart = () => {
    ...
    scrubTimerRef.current = setTimeout(() => {
      ...
      if (lastHoverRef.current) setScrubState(lastHoverRef.current)
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              {isScrubbingMode && scrubState && !scrubState.payload.isFuture && (
          ...
          {isScrubbingMode && scrubState && !scrubState.payload.isFuture && (
            <div className={`absolute top-4 z-40 p-5 bg-black/80 backdrop-blur-xl border border-white/10 shadow-2xl rounded-3xl pointer-events-none animate-in fade-in duration-200 min-w-[220px] ${scrubState.x > 180 ? 'left-4' : 'right-4'}`}>
              <div className="text-[10px] text-[var(--t3)] font-black mb-4 uppercase tracking-widest">{scrubState.payload.date}</div>
